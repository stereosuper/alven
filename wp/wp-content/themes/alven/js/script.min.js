...
function a(e,t){if(E.length&&K.length&&!W.hasClass("menu-open")&&(E.hasClass("content-career")?j.addClass("fixed"):e>E.offset().top-A-40?j.addClass("fixed"):j.removeClass("fixed"),j.hasClass("fixed")&&(t<0?j.addClass("on"):j.removeClass("on"))),ee.length&&700<k&&767<O){var n=te-A+200;ee.hasClass("single-on")?n-=400:te-A<e?ee.addClass("fixed"):ee.removeClass("fixed"),n<e&&(t<0?ee.addClass("on"):ee.removeClass("on"))}if(G.length&&(B.hasClass("single-post")||B.hasClass("page-template-default"))&&E.innerHeight()>k){var i=(e-E.offset().top)/(E.innerHeight()-k);TweenMax.set(G,{scaleX:i})}}
...
function l(e){var t=20,n=U.innerHeight(),i=E.offset().top,a=E.innerHeight();n<a+40&&n+A+20<k?postSidebarTop-A-20<=e?(i+a-20<e+n+A+20?U.css({top:a-20-n+20,width:postSidebarWidth}).removeClass("fixed").addClass("fixedBot"):U.css({top:A+20,width:postSidebarWidth}).removeClass("fixedBot").addClass("fixed"),(a+i)/2<e?(U.find("li").eq(0).find(".img").addClass("off"),U.find("li").eq(1).find(".img").addClass("on")):(U.find("li").eq(0).find(".img").removeClass("off"),U.find("li").eq(1).find(".img").removeClass("on"))):U.css({top:0,width:postSidebarWidth}).removeClass("fixed fixedBot"):U.css({top:0}).removeClass("fixed fixedBot")}
...
// Centrer cliqué
We=Ce.position().left,Be=$(".container-team").width(),je=We-Be/2+Ce.outerWidth()/2,TweenMax.to(ie,.5,{x:-je,ease:Cubic.easeOut,onComplete:m})}}))}else if(ce=Ce.offset().top-120,TweenMax.set(ie,{className:"+=member-open"}),(
// open new
...
/**** VARIABLES ****/
var H=$(document).height(),k=$(window).height(),O=window.outerWidth,D=O,I=new ScrollMagic.Controller,q=0,S=$(document).scrollTop(),N=0,W=$("html"),B=$("body"),j=$("#header"),A=j.innerHeight(),P=$("#menu-main"),E=$("#mainContent"),F=$("#main"),G=$("#readIndicator"),z=$(".btn"),L=$(".btn-invert"),K=$("#contentHeader"),R=$("#spotlightPost"),Q=!1,U=$("#postSidebar"),X=$("#related"),Y=$("#menu-responsive"),_=$("#portfolio"),J,V,Z=[],ee=$("#portfolioFilters"),te=ee.length?ee.offset().top:0,ne=$(".dropdown"),ie=$(".team"),ae=!1,oe,se,le,re,de,ce,fe=$("#fadePage"),he=$(".gallery");if(function e(){
// Scroll reveal handler
if(B.hasClass("page-template-careers")||B.hasClass("single-job")){
// Use scroll magic handle setScrollElmts
var t=$(".js-list-jobs").find("a"),n=$(".js-details-job-sidebar").find(".company-info"),i=$(".js-list-random-jobs").find("a");t.length&&r(t),n.length&&r(n),i.length&&r(i)}
// Single job tab handler
if(B.hasClass("single-job")){var o=$(".js-job-details-tab");o.length&&o.on("click","h2",function e(){if(window.innerWidth<=767){var t=$(this).parent(),a=t.index(),i=0;o.each(function e(t,n){if(t!==a){var i=$(n);i.css({maxHeight:i.find("h2").outerHeight(!0)}),i.removeClass("active")}}),t.hasClass("active")?i=$(this).outerHeight(!0):t.children().each(function e(t,n){i+=$(n).outerHeight(!0)}),t.css({maxHeight:i}),t.toggleClass("active")}})}}(),isMobile.any?W.addClass("is-mobile"):W.addClass("is-desktop"),B.hasClass("home")||a(S,0),t(),z.length&&z.each(function(e){z.eq(e).html(setBtn(z.eq(e)))}),L.length&&L.each(function(e){L.eq(e).html(setBtn(L.eq(e)))}),E.length){if(E.find("img").length){var pe=E.find("img").not(".no-scroll");B.hasClass("page-template-careers")||B.hasClass("single-job")||r(pe)}E.find(".special-cat").length&&r(E.find(".special-cat"))}if(U.length&&(postSidebarTop=U.offset().top,postSidebarWidth=U.innerWidth()),R.length&&(s(),TweenMax.set(R.find(".spotlight-post"),{y:"-120%"}),new ScrollMagic.Scene({triggerElement:"#spotlightPost"}).triggerHook(.9).setTween(TweenMax.staggerTo(R.find(".spotlight-post"),.25,{y:"0%"},.1)).addTo(I)),X.length){var me=X.find(".read-also-post");TweenMax.set(me,{opacity:0}),new ScrollMagic.Scene({triggerElement:"#related"}).triggerHook(.7).setTween(TweenMax.staggerTo(me,.25,{opacity:1},.1)).addTo(I)}if(_.length){var ge=_.find("li"),ue=ge.length,be=767<O?6:3,ve=[];window.location.search&&(
...
ve.length?p(ve):c(ge,ue,be),ee.on("click","li",h)}if(ie.length){var we=ie.find(".team-member"),$e,xe,Ce,ye,Te,Me,He,ke,Oe,De,Ie,qe,Se=$(".content-desc-responsive"),Ne,We,Be,je;u(),m(),we.on("click",function(e){e.preventDefault(),b($(this))}),$(".btn-desc > li a").on("click",function(e){e.preventDefault(),w($(this))}),$(".container-team .wrapper-btn-glob a").on("click",function(e){e.preventDefault(),x($(this))}),$(".container-team").on("click",".btn-cross",function(e){e.preventDefault(),v()})}
...
if(Y.length&&d(),$("a").length){var Ae=location.protocol+"//"+location.host,Pe=Ae+location.pathname;$("a").each(function(){var e=$(this).attr("href"),t=e.indexOf("#");if("#"!==e&&"_blank"!==$(this).attr("target"))if(0===t)$(this).addClass("scroll-anchor");else if(-1<t){var n=e.substring(0,t);"/"!==n.slice(-1)&&(n+="/"),Pe===n&&$(this).attr("href",e.substring(t)).addClass("scroll-anchor")}else 0===e.indexOf(Ae)&&$(this).addClass("fade-page-link")})}
// smooth scroll
...
// fade page effect
B.on("click",".fade-page-link",function(e){if(!$(this).hasClass("ajax-load")&&!(e.ctrlKey||e.shiftKey||e.metaKey||e.button&&1===e.button)){var t=$(this).attr("href");e.preventDefault(),TweenMax.to(fe,.2,{opacity:0,onComplete:function(){window.location.href=t}})}}),P.length){
// si le lien a la classe current menu item, c'est qu'il correspond a la page en cours
var Ee=P.find(".current-menu-item"),Fe,Ge;if(Ee.length)
