...
function a(e,t){if(E.length&&K.length&&!B.hasClass("menu-open")&&(E.hasClass("content-career")?j.addClass("fixed"):e>E.offset().top-A-40?j.addClass("fixed"):j.removeClass("fixed"),j.hasClass("fixed")&&(t<0?j.addClass("on"):j.removeClass("on"))),ee.length&&700<H&&767<O){var n=te-A+200;ee.hasClass("single-on")?n-=400:te-A<e?ee.addClass("fixed"):ee.removeClass("fixed"),n<e&&(t<0?ee.addClass("on"):ee.removeClass("on"))}if(G.length&&(W.hasClass("single-post")||W.hasClass("page-template-default"))&&E.innerHeight()>H){var i=(e-E.offset().top)/(E.innerHeight()-H);TweenMax.set(G,{scaleX:i})}}
...
function l(e){var t=20,n=U.innerHeight(),i=E.offset().top,a=E.innerHeight();n<a+40&&n+A+20<H?postSidebarTop-A-20<=e?(i+a-20<e+n+A+20?U.css({top:a-20-n+20,width:postSidebarWidth}).removeClass("fixed").addClass("fixedBot"):U.css({top:A+20,width:postSidebarWidth}).removeClass("fixedBot").addClass("fixed"),(a+i)/2<e?(U.find("li").eq(0).find(".img").addClass("off"),U.find("li").eq(1).find(".img").addClass("on")):(U.find("li").eq(0).find(".img").removeClass("off"),U.find("li").eq(1).find(".img").removeClass("on"))):U.css({top:0,width:postSidebarWidth}).removeClass("fixed fixedBot"):U.css({top:0}).removeClass("fixed fixedBot")}
...
// Centrer cliqué
Be=Ce.position().left,We=$(".container-team").width(),je=Be-We/2+Ce.outerWidth()/2,TweenMax.to(ie,.5,{x:-je,ease:Cubic.easeOut,onComplete:m})}}))}else if(ce=Ce.offset().top-120,TweenMax.set(ie,{className:"+=member-open"}),(
// open new
...
ve.length?p(ve):c(ge,ue,be),ee.on("click","li",h)}if(ie.length){var we=ie.find(".team-member"),$e,xe,Ce,ye,Te,Me,ke,He,Oe,De,Ie,qe,Se=$(".content-desc-responsive"),Ne,Be,We,je;u(),m(),we.on("click",function(e){e.preventDefault(),b($(this))}),$(".btn-desc > li a").on("click",function(e){e.preventDefault(),w($(this))}),$(".container-team .wrapper-btn-glob a").on("click",function(e){e.preventDefault(),x($(this))}),$(".container-team").on("click",".btn-cross",function(e){e.preventDefault(),v()})}
...
// fade page effect
W.on("click",".fade-page-link",function(e){if(!$(this).hasClass("ajax-load")&&!(e.ctrlKey||e.shiftKey||e.metaKey||e.button&&1===e.button)){var t=$(this).attr("href");e.preventDefault(),TweenMax.to(fe,.2,{opacity:0,onComplete:function(){window.location.href=t}})}}),P.length){
...
$("#burger").on("click",function(e){e.preventDefault(),B.toggleClass("menu-open"),B.hasClass("menu-open")?setTimeout(function(){for(var e=0,t=Y.find(".menu-small").length;e<t;e++)TweenMax.to(Y.find(".menu-title").eq(e),.25,{y:"0%",opacity:1,z:.01,force3D:!0}),TweenMax.to(Y.find(".menu-subtitle").eq(e),.25,{y:"0%",opacity:1,z:.01,force3D:!0}).delay(.05),TweenMax.staggerTo(Y.find(".menu-small").eq(e).find("li"),.25,{y:"0%",opacity:1,z:.01,force3D:!0},.08)},380):d()}),$("input").length){var ze;if($(":input").each(C).on("change",C).on("focus",function(){$(this).attr("autocomplete","on");
